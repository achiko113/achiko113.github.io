...
export default (function($) {
    ...
    // Module-load-time defaults — available immediately when the plugin module loads,
    // before any call to $.pageload({...}). Protects consumers from race conditions
    // where they reference $.pageload.options.X before the constructor runs.
    var DEFAULT_OPTIONS = {
    ...
    // Queue of stub calls that fired before the constructor ran. Each entry is
    // { path: 'fn.popup.close_popup', args: <arguments> }. Replayed on the real
    // implementations at the end of the constructor (queue-and-replay pattern).
    // Capped at 50 to prevent unbounded growth if the constructor never runs.
    var _pendingCalls = [];

    function _waitFor(methodPath) {
        return function() {
            if (_pendingCalls.length < 50) {
                _pendingCalls.push({ path: methodPath, args: arguments });
            }
        };
    }

    // Only assign $.pageload if it doesn't already exist as a function. Guards against
    // IIFE re-execution (e.g., AJAX navigation re-injecting <script> tags) which would
    // otherwise create a NEW function and lose all properties set by a previous
    // constructor call (real .options, .fn, click handler closures, etc.).
    if (typeof $.pageload !== 'function') {
    $.pageload = function(user_options) {
        // Options setup

        // Deep extend so the at-risk array/object defaults (url_params_to_retain*,
        // popstate_custom_functions) are cloned per-call. Without `true`, a shallow
        // extend would share inner references with DEFAULT_OPTIONS — any future
        // call-site that mutates an array/object option (e.g. .push) would corrupt
        // DEFAULT_OPTIONS and affect every subsequent $.pageload({...}) invocation.
        ...
        $.pageload.fn = {
            ...
            popup: {
                ...
                init: function() {
                    ...
                    $('#popup_box .close').off('click.closefunction').on('click.closefunction', function() {
                ...
                close_popup: function(forced) {
                    ...
                    if ($('body').hasClass('site-popup-fallback-forced-open') && forced_close_path && !forced) {
                            ...
                            window.location.assign(forced_close_path);
                ...
                after_load: function() {
                    ...
                    if (previous_link && typeof previous_link != 'undefined') {
                        ...
                        $('#popup_box > .inner .pagination_controls .previous').off().attr('data-href', '').addClass('disabled');
                    ...
                    if (next_link && typeof next_link != 'undefined') {
                        ...
                        $('#popup_box > .inner .pagination_controls .next').off().attr('data-href', '').addClass('disabled');
            ...
            pages: {
                ...
                add_click_events: function(element_context) {
                    ...
                    if ($.pageload.options.ajax_navigation_enabled){
                        $('a', element_context).off('click.pageload').on('click.pageload', function() {
...
// to be moved into other js file
           $('.buy_enquire_button .button').off('click').on('click', function () {
           ...
           $(".buy_enquire_overlay, .buy_enquire_quick_view .close").off('click').on('click', function () {
                    h.accessibility.on_popup_closing();
                    ...
                    if (new_page_popup_mode_enabled && !$('body').hasClass('site-popup-fallback-mode')) { // Skip this step if we are on a full page where the content usually opens as popup content, continue to open in fallback mode - not in a popup
                        ...
                        if ($.pageload.options.content_load_adjacent) {
                            ...
                            // This allows two pages to be visible at the same time and transition between them
                                ...
                                $(window).off();
                                $('#container *, #cms-frontend-toolbar *').off().off();
        ...
        //$(window).ready(function() {
            ...
            // Replay any calls queued by _waitFor() stubs before this constructor ran.
            // Each entry resolves its dotted path against $.pageload and invokes the
            // (now real) function with the original arguments. Failures swallowed —
            // a stub call that becomes invalid by replay time shouldn't break init.
            while (_pendingCalls.length > 0) {
                var _call = _pendingCalls.shift();
                var _parts = _call.path.split('.');
                var _target = $.pageload;
                for (var _i = 0; _i < _parts.length - 1 && _target; _i++) {
                    _target = _target[_parts[_i]];
                }
                if (_target) {
                    var _realFn = _target[_parts[_parts.length - 1]];
                    if (typeof _realFn === 'function') {
                        try { _realFn.apply(_target, _call.args); } catch (e) { /* swallow */ }
                    }
                }
            }
        //});
    ...
    } // close the if (typeof $.pageload !== 'function') guard

    // Module-load-time stubs — populate $.pageload.options, $.pageload.fn, and top-level
    // methods with safe defaults/no-ops so references before $.pageload({...}) is called
    // don't crash. The constructor REPLACES these with real implementations when invoked.
    //
    // Idempotent: only set stubs where the property doesn't already exist. This guards
    // against IIFE re-execution (e.g., AJAX navigation re-injecting the script tag), which
    // would otherwise overwrite real implementations from a previous constructor call.
    // Deep extend so nested array/object defaults are cloned (see comment in the constructor)
    if (!$.pageload.options) $.pageload.options = $.extend(true, {}, DEFAULT_OPTIONS);
    if (!$.pageload.fn) $.pageload.fn = {
        init: _waitFor('fn.init'),
        _fetchBackgroundImages: _waitFor('fn._fetchBackgroundImages'),
        _decryptContent: function() { return ''; },  // returns a value, keep as no-op (can't queue+return synchronously)
        _replace_main_content: _waitFor('fn._replace_main_content'),
        preloader: {
            init: _waitFor('fn.preloader.init'),
            start_load: _waitFor('fn.preloader.start_load'),
            preload_images: _waitFor('fn.preloader.preload_images'),
            load_complete: _waitFor('fn.preloader.load_complete'),
            start_ajax: _waitFor('fn.preloader.start_ajax'),
            end_ajax: _waitFor('fn.preloader.end_ajax')
        },
        popup: {
            settings: { fallback_close_path: '' },
            init: _waitFor('fn.popup.init'),
            open: _waitFor('fn.popup.open'),
            close_popup: _waitFor('fn.popup.close_popup'),
            after_load: _waitFor('fn.popup.after_load')
        },
        slidein: {
            init: _waitFor('fn.slidein.init'),
            slidein_after: _waitFor('fn.slidein.slidein_after')
        },
        pages: {
            init: _waitFor('fn.pages.init'),
            add_click_events: _waitFor('fn.pages.add_click_events'),
            load: _waitFor('fn.pages.load'),
            after_load: _waitFor('fn.pages.after_load'),
            track_pageview: _waitFor('fn.pages.track_pageview')
        }
    };
    if (!$.pageload.load) $.pageload.load = _waitFor('load');
    if (!$.pageload.refresh) $.pageload.refresh = _waitFor('refresh');
    if (!$.pageload.popup_close) $.pageload.popup_close = _waitFor('popup_close');
    if (!$.pageload.popup_open) $.pageload.popup_open = _waitFor('popup_open');
    if (!$.pageload.slidein) $.pageload.slidein = _waitFor('slidein');
